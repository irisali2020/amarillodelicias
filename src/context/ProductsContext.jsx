...
export const ProductsProvider = ({ children }) => {
  ...
  useEffect(() => {
    const cargarProductos = async () => {
      try {
        const respuesta = await fetch('https://68dad96c23ebc87faa316b2d.mockapi.io/api/productos');
  ...
  const editarProducto = async (productoActualizado) => {
    try {
      const respuesta = await fetch(`https://https://68dad96c23ebc87faa316b2d.mockapi.io/api/productos/${productoActualizado.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
